...
keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(text="course deal", url="https://t.me/+OprOK_dzoag4M2Fl"),
            InlineKeyboardButton(text="backup", url="https://t.me/+IVcD0g1xRyhmNjhl"),
        ],
    ]
)
...
async def txt_handler(client: Client, m: Message):
    await bot.send_message(m.chat.id, text= (
        "🎉Congrats! You are using @AllCourseADMIN_BOT\n┣\n"
        ...
        "**If you have any questions, feel free to ask! @AllCourseADMIN_BOT💬**"
        )
    ) 
...
async def txt_handler(bot: Client, m: Message):
    ...
    credit = "️@AllCourseADMIN_BOT 🕊️⁪⁬⁮⁮⁮"
    if raw_text3 == '1':
        CR = '@AllCourseADMIN_BOT 🕊️'
    elif raw_text3:
        CR = raw_text3
    else:
        CR = credit
            ...
            try:  
                ...
                if "drive" in url:
                    ...
                    Show = f"🚀𝐏𝐑𝐎𝐆𝐑𝐄𝐒𝐒 » {progress:.2f}%\n┃\n" \
                           ...
                           f"✦𝐁𝐨𝐭 𝐌𝐚𝐝𝐞 𝐁𝐲 ✦ `@AllCourseADMIN_BOT🐦`"
    ...
    await m.reply_text("<pre><code>Downloaded By ⌈✨『@AllCourseADMIN_BOT』✨⌋</code></pre>")
...
async def txt_handler(bot: Client, m: Message):
    ...
    try: 
        for i in range(arg-1, len(links)):
            ...
            try:  
                ...
                if ".pdf" in url or "drive" in url or ".jpg" in url or ".jpeg" in url or ".png" in url:
                    ...
                    Show = f"<pre><code>⚡Dᴏᴡɴʟᴏᴀᴅ Sᴛᴀʀᴛᴇᴅ...⏳\n✦𝐁𝐨𝐭 𝐌𝐚𝐝𝐞 𝐁𝐲 ✦ @AllCourseADMIN_BOT</code></pre>"
                    prog = await m.reply_text(Show, disable_web_page_preview=True)
                    res_file = await helper.download_video(url, cmd, name)
                    filename = res_file
                    await prog.delete(True)
        ...
        await m.reply_text(e)
    await m.reply_text("<pre><code>Converted By ⌈✨『@AllCourseADMIN_BOT』✨⌋</code></pre>")
    ...
    try:
        ...
        # Check if the response status is OK
            ...
            # Remove the PDF file after sending
                    ...
                    emoji_message = await show_random_emojis(message)
                    Show = f"<pre><code>**⚡Dᴏᴡɴʟᴏᴀᴅ Sᴛᴀʀᴛᴇᴅ...⏳**</code></pre>\n🔗𝐋𝐢𝐧𝐤 » `{link}`\n<pre><code>✦𝐁𝐨𝐭 𝐌𝐚𝐝𝐞 𝐁𝐲 ✦ `@AllCourseADMIN_BOT`</code></pre>"
